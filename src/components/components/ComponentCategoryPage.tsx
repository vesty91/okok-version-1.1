import React, { useState, useEffect, useRef } from 'react';
    ...
    const ComponentCategoryPage = () => {
      ...
      const handleCloseMoreFilters = (e: MouseEvent) => {
      ...
      useEffect(() => {
        ...
        document.addEventListener('mousedown', handleCloseMoreFilters);
        return () => {
          ...
          document.removeEventListener('mousedown', handleCloseMoreFilters);
        };
      }, []);
            ...
            <div className="space-y-4">
              ...
              <div className="relative" ref={moreFiltersRef}>
                <button
                  ...
                  className="bg-blue-500 text-white py-2 px-4 rounded-md text-sm w-full text-left"
                >
                  + DE FILTRES
                </button>
                {isMoreFiltersOpen && (
                  <div className="absolute left-0 mt-1 w-full bg-white rounded-md shadow-lg z-10 p-4 space-y-4">
                    <div>
                    ...
                    <div>
                      <label className="block text-gray-700 mb-2 text-sm">UTILISATION TYPE</label>
                      <div className="space-y-1">
                        {usageTypes.map(usage => (
                          <div
                            key={usage}
                            onClick={() => handleUsageTypeClick(usage)}
                            className={`px-3 py-2 text-gray-800 hover:bg-gray-100 cursor-pointer ${selectedUsageTypes.includes(usage) ? 'bg-gray-100' : ''}`}
                          >
                            {usage}
                          </div>
                        ))}
                      </div>
                      {selectedUsageTypes.length > 0 && (
                        <div className="p-2 border-t border-gray-200">
                          <button onClick={handleClearAllUsageTypes} className="text-red-500 hover:text-red-700 text-sm">
                            Clear All
                          </button>
                        </div>
                      )}
